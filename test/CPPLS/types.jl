@testset "CPPLS stores selected training artefact" begin
    configs = [
        (Float32, Int8, (; n_samples=5, n_predictors=4, n_responses=3, n_components=2)),
        (Float64, Int16, (; n_samples=3, n_predictors=2, n_responses=2, n_components=1)),
    ]
    ...
    for (T, Tmask, dims) in configs
        ...
        regression_coefficients = reshape(T.(1:(n_predictors * n_responses * n_components)),
        ...
        X_loading_weights = reshape(T.(101:(100 + n_predictors * n_components)),
                                    n_predictors, n_components)
        Y_scores = reshape(T.(1:(n_samples * n_components)), n_samples, n_components)
        Y_loadings = reshape(T.(51:(50 + n_responses * n_components)),
                             n_responses, n_components)
        projection = reshape(T.(11:(10 + n_predictors * n_components)),
                             n_predictors, n_components)
        X_means = reshape(T.(1:n_predictors), 1, n_predictors)
        Y_means = reshape(T.(1:n_responses), 1, n_responses)
        fitted_values = reshape(T.(1:(n_samples * n_responses * n_components)),
                                n_samples, n_responses, n_components)
        residuals = reshape(T.(401:(400 + n_samples * n_responses * n_components)),
                            n_samples, n_responses, n_components)
        ...
        small_norm_indices = reshape(Tmask.(0:(n_components * n_predictors - 1)),
                                     n_components, n_predictors)
        canonical_coefficients = reshape(T.(21:(20 + n_responses * n_components)),
                                         n_responses, n_components)

        cppls = CPPLS(regression_coefficients, X_scores, X_loadings, X_loading_weights,
                      Y_scores, Y_loadings, projection, X_means, Y_means, fitted_values,
                      residuals, X_variance, X_total_variance, gammas,
                      canonical_correlations, small_norm_indices, canonical_coefficients)

        @test cppls isa StatisticalProjections.AbstractCPPLS
        @test cppls isa CPPLS{T, Tmask}
        ...
        @test size(cppls.regression_coefficients) == (n_predictors, n_responses, n_components)
...
@testset "CPPLSLight keeps prediction essentials" begin
    ...
    for (T, dims) in configs
        ...
        light_model = CPPLSLight(regression_coefficients, X_means, Y_means)
        ...
        @test size(light_model.regression_coefficients) == (n_predictors, n_responses, n_components)
